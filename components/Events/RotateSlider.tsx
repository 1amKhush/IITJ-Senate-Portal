  ...
  return (
    <div className="relative w-full h-screen overflow-hidden text-center bg-gradient-to-br from-navy via-navy-light to-navy">
      ...
      <div className="absolute inset-0 flex items-center overflow-hidden z-10">
        ...
        <div className="flex animate-scroll-x gap-4 hover:[animation-play-state:paused]" style={{ width: 'max-content' }}>
          {/* First set of images */}
          {[...Array(imageCount)].map((_, i) => (
            <div
              key={`first-${i}`}
              className="flex-shrink-0 w-[250px] h-[340px] sm:w-[280px] sm:h-[380px] lg:w-[320px] lg:h-[440px] group"
            >
      ...
      {/* Foreground Content */}
      <div className="absolute bottom-0 left-0 w-full p-6 sm:p-10 flex flex-col sm:flex-row items-center sm:items-end justify-between z-30 text-center sm:text-left">
        <h1 className="font-['ICA_Rubrik'] text-6xl sm:text-8xl md:text-9xl lg:text-[10rem] leading-none text-white relative drop-shadow-2xl">
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-gray-300">EVENTS</span>
        </h1>
        <div className="max-w-xs md:max-w-md mt-6 sm:mt-0 sm:ml-8 text-white z-10">
          <div className="glass-card rounded-2xl p-6 backdrop-blur-xl">
            <h2 className="text-xl sm:text-2xl font-bold gradient-text">Events at IITJ</h2>
            <p className="font-semibold text-fulvous text-lg mt-1">Culture • Tech • Sports</p>
